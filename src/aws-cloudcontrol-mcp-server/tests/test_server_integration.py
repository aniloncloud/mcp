...
async def wait_for_operation_completion(ctx, request_token, timeout=300, interval=5):
    ...
    while time.time() - start_time < timeout:
        ...
        if operation_status == "SUCCESS":
            logger.info(
                f"Operation completed successfully after {time.time() - start_time:.1f} seconds"
            )
            return status_result
...
async def test_list_resource_types(ctx):
    ...
    for i, type_summary in enumerate(type_summaries[:5]):
        logger.info(
            f"Resource Type {i + 1}: {type_summary.get('TypeName')} - {type_summary.get('Description', 'No description')}"
        )
...
async def test_logs_loggroup_lifecycle(ctx, test_resource_prefix):
    ...
    create_result = await create_resource(
        ...
        desired_state={"LogGroupName": log_group_name, "RetentionInDays": retention_days},
    ...
    get_result = await get_resource(ctx, type_name="AWS::Logs::LogGroup", identifier=log_group_name)
    ...
    patch_document = json.dumps(
        [{"op": "replace", "path": "/RetentionInDays", "value": new_retention_days}]
    )
    ...
    assert properties.get("RetentionInDays") == new_retention_days, (
        "Updated retention days doesn't match"
    )
...
async def test_ssm_parameter_lifecycle(ctx, test_resource_prefix):
    ...
    get_result = await get_resource(ctx, type_name="AWS::SSM::Parameter", identifier=parameter_name)
    ...
    properties = get_result["ResourceDescription"]["Properties"]
    logger.info(
        f"SSM parameter properties: Name={properties.get('Name')}, Type={properties.get('Type')}, Description={properties.get('Description')}"
    )
    ...
    logger.info(
        f"Expected description: {parameter_description}, Actual description: {properties.get('Description')}"
    )
    # Note: We don't log the actual value for security reasons

    # Step 3: Update the parameter
    new_parameter_value = "test-value-2"
    logger.info("Updating SSM parameter value")
    patch_document = json.dumps([{"op": "replace", "path": "/Value", "value": new_parameter_value}])
...
async def test_secrets_manager_lifecycle(ctx, test_resource_prefix):
    ...
    get_result = await get_resource(
        ctx, type_name="AWS::SecretsManager::Secret", identifier=secret_arn
    )
    ...
    properties = get_result["ResourceDescription"]["Properties"]
    logger.info(
        f"Secret properties: Name={properties.get('Name')}, Description={properties.get('Description')}"
    )
...
async def test_resource_request_management(ctx, test_resource_prefix):
    ...
    create_result = await create_resource(
        ctx,
        type_name="AWS::Logs::LogGroup",
        desired_state={"LogGroupName": log_group_name, "RetentionInDays": 1},
    ...
    logger.info("Successfully listed resource requests")
...
async def test_idempotency_with_client_token(ctx, test_resource_prefix):
    ...
    logger.info(
        f"Attempting to create the same log group with the same client token: {client_token}"
    )
    ...
    assert identifier_1 == identifier_2, (
        "Idempotent operations should return the same resource identifier"
    )
