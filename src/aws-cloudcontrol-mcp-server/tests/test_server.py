...
@pytest.mark.asyncio
async def test_create_resource_with_optional_params(mock_boto3_client, mock_context, mock_progress_event):
...
async def test_get_resource_client_error(mock_boto3_client, mock_context):
    ...
    # Set up mock error
    mock_error = MagicMock()
    mock_error.response = {'Error': {'Code': 'ResourceNotFoundException', 'Message': 'Resource not found'}}
...
@pytest.mark.asyncio
async def test_update_resource_with_optional_params(mock_boto3_client, mock_context, mock_progress_event):
...
async def test_delete_resource(mock_boto3_client, mock_context, mock_progress_event):
    ...
    assert 'ProgressEvent' in result
    assert result['ProgressEvent']['TypeName'] == type_name
    assert result['ProgressEvent']['Operation'] == 'CREATE'  # This would be DELETE in a real response
    assert result['ProgressEvent']['Identifier'] == 'TestLogGroup'
...
@pytest.mark.asyncio
async def test_delete_resource_with_optional_params(mock_boto3_client, mock_context, mock_progress_event):
...
async def test_get_resource_request_status(mock_boto3_client, mock_context, mock_progress_event):
    ...
    # Set up mock response
    mock_boto3_client.get_resource_request_status.return_value = {'ProgressEvent': mock_progress_event}
    
    # Patch the cloudcontrol_client in the server module
    with patch('awslabs.aws_cloudcontrol_server.server.cloudcontrol_client') as mock_client:
        mock_client.cloudcontrol_client = mock_boto3_client
        result = await get_resource_request_status(
            mock_context, 
...
async def test_get_resource_request_status_client_error(mock_boto3_client, mock_context):
    ...
    # Set up mock error
    mock_error = MagicMock()
    mock_error.response = {'Error': {'Code': 'ResourceRequestNotFound', 'Message': 'Request not found'}}
...
async def test_cancel_resource_request(mock_boto3_client, mock_context, mock_progress_event):
    ...
    with patch('awslabs.aws_cloudcontrol_server.server.cloudcontrol_client') as mock_client:
        mock_client.cloudcontrol_client = mock_boto3_client
        result = await cancel_resource_request(
            mock_context, 
            request_token=request_token
        )
...
@pytest.mark.asyncio
async def test_list_resource_requests_with_optional_params(mock_boto3_client, mock_context, mock_progress_event):
...
def test_cloudcontrol_client_initialization_with_session_token(monkeypatch):
    ...
    monkeypatch.setenv('AWS_ACCESS_KEY_ID', 'AKIAIOSFODNN7EXAMPLE')
    monkeypatch.setenv('AWS_SECRET_ACCESS_KEY', 'wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY')
    monkeypatch.setenv('AWS_SESSION_TOKEN', 'AQoEXAMPLEH4aoAH0gNCAPyJxz4BlCFFxWNE1OPTgk5TthT+FvwqnKwRcOIfrRh3c/LTo6UDdyJwOOvEVPvLXCrrrUtdnniCEXAMPLE/IvU1dYUg2RVAJBanLiHb4IgRmpRV3zrkuWJOgQs8IZZaIv2BXIa2R4Olgk')
    ...
    with patch('boto3.client') as mock_boto3_client:
        ...
        assert kwargs['aws_access_key_id'] == 'AKIAIOSFODNN7EXAMPLE'
        assert kwargs['aws_secret_access_key'] == 'wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY'
        assert kwargs['aws_session_token'] == 'AQoEXAMPLEH4aoAH0gNCAPyJxz4BlCFFxWNE1OPTgk5TthT+FvwqnKwRcOIfrRh3c/LTo6UDdyJwOOvEVPvLXCrrrUtdnniCEXAMPLE/IvU1dYUg2RVAJBanLiHb4IgRmpRV3zrkuWJOgQs8IZZaIv2BXIa2R4Olgk'
